    ...
    if (response.success === false) {
      ...
      var data = response.data;
        ...
        for (var i = 0; i < data.length ; i++) {
          $("#tableReference").DataTable().row.add([
            data[i].brand,
            data[i].model,
            data[i].series,
            data[i].date,
            '<a href="?p=10&id_eqp=' + data[i].id_eqp + '&id_client=' + data[i].id_client + '"> <button type="button" class="btn btn-outline-success" name="button"><i class="fas fa-pencil-alt"></i></button></a>'
          ]);
        }
        $("#tableReference").DataTable().draw(true);
    }
  });
}
