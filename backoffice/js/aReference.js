    ...
    if (response.success === false) {
      ...
      var data = response.data;
        ...
        for (var i = 0; i < data.length ; i++) {
          $("#tableaReference").DataTable().row.add([
            data[i].brand,
            data[i].model,
            data[i].series,
            data[i].date,
            '<a href="?p=22&id_eqp=' + data[i].id_eqp + '&id_client=' + data[i].id_client + '"> <button type="button" class="btn btn-outline-success" name="button"><i class="bi bi-card-text"></i></button></a>'
          ]);
        }
        $("#tableaReference").DataTable().draw(true);
    }
  });
}
